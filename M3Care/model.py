...
from m3care.transformer.model import NMTTransformer, MMTransformerEncoder
from m3care.utils import init_weights, clones, length_to_mask, guassian_kernel
from m3care.transformer.encode import PositionalEncoding
from m3care.graph import GraphConvolution
...
class M3Care(nn.Module):
    def __init__(self, input_dim, hidden_dim, embed_size, vocab, device, output_dim=1, keep_prob=1):
        ...
        self.NLP_model = NMTTransformer(embed_size=embed_size,
        ...
        self.MM_model = MMTransformerEncoder(input_dim = self.hidden_dim, d_model = self.hidden_dim,  \
                                                      MHD_num_head = 4 , d_ff = self.hidden_dim*4, output_dim = 1).to(device)
        self.MM_model2 = MMTransformerEncoder(input_dim = self.hidden_dim, d_model = self.hidden_dim,  \
    ...
    def forward(self, input, left_fundus, right_fundus, left_diag, right_diag, l_r_masks):
        ...
        left_diag_mask = length_to_mask(torch.from_numpy(np.array(left_diag_lens))).unsqueeze(1).to(self.device)
        ...
        left_diag_mask_ = torch.from_numpy(np.array(l_r_masks[0])).to(self.device).unsqueeze(1)# b 1
        right_diag_mask_ = torch.from_numpy(np.array(l_r_masks[1])).to(self.device).unsqueeze(1)
        ...
        left_diag_contexts = self.PositionalEncoding(left_diag_contexts_)  +  \
            self.token_type_embeddings(torch.ones_like(left_diag_mask.permute(0,2,1).squeeze(-1)).to(self.device).long())

        values_hidden = values_hidden.unsqueeze(1)  +  \
            self.token_type_embeddings(torch.zeros_like(val_mask.permute(0,2,1).squeeze(-1)).to(self.device).long())

        right_diag_contexts = self.PositionalEncoding(right_diag_contexts_)  +  \
            self.token_type_embeddings(2*torch.ones_like(right_diag_mask.permute(0,2,1).squeeze(-1)).to(self.device).long())
        
        left_f_contexts = left_f.unsqueeze(1)  +  \
            self.token_type_embeddings(3* torch.ones_like(left_f_mask.permute(0,2,1).squeeze(-1)).to(self.device).long())

        right_f_contexts = right_f.unsqueeze(1)  +  \
            self.token_type_embeddings(4* torch.ones_like(right_f_mask.permute(0,2,1).squeeze(-1)).to(self.device).long())
