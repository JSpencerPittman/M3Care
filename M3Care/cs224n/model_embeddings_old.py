...
class ModelEmbeddings(nn.Module):
    ...
    def __init__(self, embed_size, vocab):
        ...
        self.vocab = vocab
        self.embeddings = nn.Embedding(len(vocab.char2id), self.char_embed_size)
    ...
    def forward(self, input):
        ...
        print(f"TAKES IN {len(self.vocab.char2id)} and returns {self.char_embed_size}")

        print("DATA SHAPE0: ", input.shape)
        print(input[0][0])
        data = self.embeddings(input) # (batch_size, sentence_length, max_word_length, char_embed_size)
        print("DATA SHAPE1: ", input.shape)
        data_shape = data.shape
        print("DATA SHAPE2: ", input.shape)
